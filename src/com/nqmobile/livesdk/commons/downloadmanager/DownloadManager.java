...
public class DownloadManager {
	...
	private static DownloadManager sDlMgr;
	 ...
	 * {@link com.nqmobile.live.common.downloadmanager.NotificationManager}. By default,
	...
	public synchronized static DownloadManager getInstance(ContentResolver resolver, String packageName) {
		if (sDlMgr == null) {
			sDlMgr = new DownloadManager(resolver, packageName);
		}
		return sDlMgr;
	}
	...
	try {
		...
		//not sure if STATUS_PENDING is need
...
//		}
	...
	values.put(Downloads.COLUMN_NO_INTEGRITY, 1);
	if (ids.length == 1) {
		mResolver.update(ContentUris.withAppendedId(mBaseUri, ids[0]), values,
				null, null);
	}else{
    ...
    public void modifyDownloadNetwork(int networkType, long... ids) {
    	ContentValues values = new ContentValues();
    	//用户如果手动恢复，则设置允许网络为3g wifi均可
    	values.put(Downloads.COLUMN_ALLOWED_NETWORK_TYPES, networkType);
    	if (ids.length == 1) {
    		mResolver.update(ContentUris.withAppendedId(mBaseUri, ids[0]), values,
    				null, null);
    	}else{
	...
	ContentValues values = new ContentValues();
	//not sure status change to STATUS_PENDING or STATUS_RUNNING
	//values.put(Downloads.COLUMN_STATUS, Downloads.STATUS_RUNNING);
	values.put(Downloads.COLUMN_STATUS, Downloads.STATUS_PENDING);
	values.put(Downloads.COLUMN_CONTROL, Downloads.CONTROL_RUN);
	//用户如果手动恢复，则设置允许网络为3g wifi均可
	//values.put(Downloads.COLUMN_ALLOWED_NETWORK_TYPES, DownloadManager.Request.NETWORK_MOBILE | DownloadManager.Request.NETWORK_WIFI);
	if (ids.length == 1) {
		mResolver.update(ContentUris.withAppendedId(mBaseUri, ids[0]), values,
				null, null);
	}else{
