...
public class DownloadInfo {
    ...
    private boolean isReadyToStart(long now) {   
        if (mHasActiveThread) {
            // already running
        	NqLog.i("isReadyToStart return false: already running!");
        	
        	// fix bug 多次暂停/恢复之后概率中出现等待中的情况
        	if (mStatus == Downloads.STATUS_PAUSED_BY_APP) {
        ...
        if (mControl == Downloads.CONTROL_PAUSED) {
            ...
            // already running
        	NqLog.i("isReadyToStart return false: mControl == Downloads.CONTROL_PAUSED!");        	
            return false;
        }
        switch (mStatus) {
    ...
    private int checkIsNetworkTypeAllowed(int networkType) {
    	NqLog.i("checkIsNetworkTypeAllowed: mIsPublicApi = " + mIsPublicApi);
        if (mIsPublicApi) {
    ...
    void startIfReady(long now, Collection<DownloadInfo> DownloadInfo) {
        if (!isReadyToStart(now)) {
            return;
        }

    	convert2PendingStatus();
        // 如果超过最大并发下载数，不启动下载
        if (isExceedMaxThreadsNumber(DownloadInfo)) {
        	return;
        }
        
        // 如果静默下载超过最大允许线程数的一半则不启动
        if (isExceedMaxSilentThreadNumber(DownloadInfo)) {
        	return;
        }
        ...
        NqLog.i("startIfReady: startThread!"); 
    }
    ...
    private void convert2PendingStatus() {
        if (mStatus == Downloads.STATUS_QUEUED_FOR_WIFI || mStatus == Downloads.STATUS_WAITING_FOR_NETWORK) {
    ...
    private boolean isExceedMaxThreadsNumber(Collection<DownloadInfo> downloadInfoList) {
        //控制同时下载不能超过最大并发下载数
        if (mStatus == Downloads.STATUS_PENDING) {       	
	        ...
	        for (DownloadInfo info: downloadInfoList) {
	        	if (info.mStatus == Downloads.STATUS_RUNNING) {
	        		total_running++;	
	        		if (total_running >= Downloads.MAX_THREADS_RUNNING) {
	        			return true;
	        		}
	        	}
	        }
    ...
    private boolean isExceedMaxSilentThreadNumber(Collection<DownloadInfo> downloadInfoList) {
        //控制静默下载不能超过最大并发下载数的一半
        if (!isActiveAndVisible(this) && mStatus == Downloads.STATUS_PENDING) {       	
        	int total_running = 0;
	        for (DownloadInfo info: downloadInfoList) {
	        	if (info.mStatus == Downloads.STATUS_RUNNING && !isActiveAndVisible(info)) {
	        		total_running++;	
	        		if (total_running >= Downloads.MAX_THREADS_RUNNING/2) {
	        			return true;
	        		}
	        	}
	        }
    ...
    private boolean isActiveAndVisible(DownloadInfo download) {
        return download.mVisibility != Downloads.VISIBILITY_HIDDEN;
    }
}
