...
package com.nqmobile.livesdk.commons.downloadmanager.ui;
...
import com.nqmobile.livesdk.commons.downloadmanager.ui.DownloadItem.DownloadSelectListener;
...
*/
  /*    @Override
	...
	*/
    	...
    	else if (item.getItemId() == R.id.download_menu_sort_by_date){
    	    mIsSortedBySize = false;
    	    chooseListToShow();
    	    return true;
    	}
...
*/
    if(v.getId() == R.id.selection_delete) {
	    for (Long downloadId : mSelectedIds) {
		deleteDownload(downloadId);
	    }
	    clearSelection();
    }else if (v.getId() == R.id.deselect_all){
	    clearSelection();
	}else{}
    }
