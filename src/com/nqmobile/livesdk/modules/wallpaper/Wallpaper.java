...
public class Wallpaper implements Serializable {
    ...
    public Wallpaper(TWallpaperResource resource, Context context){
    	if (resource != null) {
            setStrId(StringUtil.nullToEmpty(resource.getResourceId()));
            setStrName(resource.getName());
            setIntType(resource.getType());
            setStrAuthor(StringUtil.nullToEmpty(resource.getAuthor()));
            setStrVersion(StringUtil.nullToEmpty(resource.getVersion()));
            setStrSource(StringUtil.nullToEmpty(resource.getSource()));
            setLongSize(resource.getSize());
            setLongDownloadCount(resource.getDownloadNum());
            setStrIconUrl(StringUtil.nullToEmpty(resource.getIcon()));
            setStrWallpaperUrl(StringUtil.nullToEmpty(resource.getPicture()));
            setPreviewPicture(StringUtil.nullToEmpty(resource.getPreviewPicture()));
            setDailyIcon(StringUtil.nullToEmpty(resource.getDailyIcon()));
            ...
            if (!TextUtils.isEmpty(getStrWallpaperUrl())) {
                ...
                setStrWallpaperPath(new StringBuilder().append(strPath).append(getStrId())
                        .append(StringUtil.getExt(getStrWallpaperUrl())).toString());
            }
            
            //wallpaper预览图路径
            if(!TextUtils.isEmpty(getPreviewPicture())) {
            	String strPath = new StringBuilder().append(SDCardPath).append(WallpaperConstants.STORE_IMAGE_LOCAL_PATH_WALLPAPER).toString();
            	setPreviewPicturePath(new StringBuilder().append(strPath).append(getStrId()).append("_preview")
                .append(StringUtil.getExt(getPreviewPicture())).toString());
            }
	...
	public String toString() {
		return "Wallpaper [strId=" + strId + ", intSourceType=" + intSourceType
				+ ", intType=" + intType + ", strAuthor=" + strAuthor
				+ ", strName=" + strName + ", strVersion=" + strVersion
				+ ", strSource=" + strSource + ", longSize=" + longSize
				+ ", longDownloadCount=" + longDownloadCount + ", strIconUrl="
				+ strIconUrl + ", strWallpaperUrl=" + strWallpaperUrl
				+ ", strIconPath=" + strIconPath + ", strWallpaperPath="
				+ strWallpaperPath + ", longUpdateTime=" + longUpdateTime
				+ ", longLocalTime=" + longLocalTime + ", preViewUrl=" + previewPicture +
                ", dailyIcon=" + dailyIcon + "]";
	}
